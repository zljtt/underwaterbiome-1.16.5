...
public class WaterWorldChunkGenerator extends ChunkGenerator {
	public static final Codec<WaterWorldChunkGenerator> CODEC = RecordCodecBuilder.create((c) -> {
		...
		})).apply(c, c.stable(WaterWorldChunkGenerator::new));
	});
	...
	public WaterWorldChunkGenerator(BiomeProvider biomeProvider, Supplier<DimensionSettings> dimensionSettings) {
		this(biomeProvider, biomeProvider, (new Random()).nextLong(), dimensionSettings); // Set random seed
	}

	@SuppressWarnings("deprecation")
	private WaterWorldChunkGenerator(BiomeProvider biomeProvider1, BiomeProvider biomeProvider2, long seed,
	...
	private double sampleAndClampNoise(int p_222552_1_, int p_222552_2_, int p_222552_3_, double p_222552_4_, double p_222552_6_,
			double p_222552_8_, double p_222552_10_) {
		double d0 = 0.0D;
		double d1 = 0.0D;
		double d2 = 0.0D;
//		boolean flag = true;
		...
		for (int i = 0; i < 16; ++i) {
			...
			if (i < 8) {
				...
				if (improvednoisegenerator2 != null) {
					d2 += improvednoisegenerator2.noise(OctavesNoiseGenerator.wrap((double) p_222552_1_ * p_222552_8_ * d3),
							OctavesNoiseGenerator.wrap((double) p_222552_2_ * p_222552_10_ * d3),
							OctavesNoiseGenerator.wrap((double) p_222552_3_ * p_222552_8_ * d3), p_222552_10_ * d3,
							(double) p_222552_2_ * p_222552_10_ * d3) / d3;
				}
			}
...
//			int i = 2;
	...
	private int iterateNoiseColumn(int p_236087_1_, int p_236087_2_, @Nullable BlockState[] p_236087_3_,
			@Nullable Predicate<BlockState> p_236087_4_) {
		...
		double[][] adouble = new double[][] { this.makeAndFillNoiseColumn(i, j), this.makeAndFillNoiseColumn(i, j + 1),
				this.makeAndFillNoiseColumn(i + 1, j), this.makeAndFillNoiseColumn(i + 1, j + 1) };
...
//		double d0 = 0.0625D;
		...
		for (int i1 = 0; i1 < 16; ++i1) {
			for (int j1 = 0; j1 < 16; ++j1) {
				...
				double d1 = this.surfaceNoise.getSurfaceNoiseValue((double) k1 * 0.0625D, (double) l1 * 0.0625D, 0.0625D,
						(double) i1 * 0.0625D) * 15.0D;
				p_225551_1_.getBiome(blockpos$mutable.set(k + i1, i2, l + j1)).buildSurfaceAt(sharedseedrandom, p_225551_2_, k1,
						l1, i2, d1, this.defaultBlock, this.defaultFluid, this.getSeaLevel(), p_225551_1_.getSeed());
			}
		}
...
//		int i1 = 5;
				...
				if (flag) {
					for (int j1 = 0; j1 < 5; ++j1) {
						if (j1 <= p_222555_2_.nextInt(5)) {
							p_222555_1_.setBlockState(blockpos$mutable.set(blockpos.getX(), l - j1, blockpos.getZ()),
				...
				if (flag1) {
					for (int k1 = 4; k1 >= 0; --k1) {
						if (k1 <= p_222555_2_.nextInt(5)) {
							p_222555_1_.setBlockState(blockpos$mutable.set(blockpos.getX(), k + k1, blockpos.getZ()),
									Blocks.BEDROCK.defaultBlockState(), false);
						}
					}
				}
				...
				for (StructurePiece structurepiece1 : p_236089_5_.getPieces()) {
					if (structurepiece1.isCloseToChunk(chunkpos, 12)) {
						if (structurepiece1 instanceof AbstractVillagePiece) {
							...
							JigsawPattern.PlacementBehaviour jigsawpattern$placementbehaviour = abstractvillagepiece.getElement()
									.getProjection();
						...
						for (int l2 = 0; l2 < this.chunkWidth; ++l2) {
							...
							for (int k3 = 0; k3 < this.chunkWidth; ++k3) {
								...
								for (d18 = d18 / 2.0D - d18 * d18 * d18 / 24.0D; objectlistiterator
										.hasNext(); d18 += getContribution(j4, k4, l4) * 0.8D) {
									...
									k4 = i2 - (mutableboundingbox.y0 + (structurepiece instanceof AbstractVillagePiece
											? ((AbstractVillagePiece) structurepiece).getGroundLevelDelta()
											: 0));
	...
	public List<MobSpawnInfo.Spawners> getMobsAt(Biome p_230353_1_, StructureManager p_230353_2_,
			EntityClassification p_230353_3_, BlockPos p_230353_4_) {
		List<MobSpawnInfo.Spawners> spawns = net.minecraftforge.common.world.StructureSpawnManager.getStructureSpawns(p_230353_2_,
				p_230353_3_, p_230353_4_);
		if (spawns != null)
			return spawns;
